...
from OneEncoderTwoDecoderTransformer import *
from ClassificationDatasetFromDict import *

logging.basicConfig(filename='classification_2_0_fp32_training.log', filemode='a', level=logging.INFO, format = '{asctime} {filename} {message}', style="{")
...
model = OneEncoderTwoDecoderTransformer(
    vocab_size, pad_id, CLS_label_id, num_pos=seq_len
)
...
for epoch in trange(EPOCHS_START, EPOCHS+EPOCHS_START):
    ...
    for ix, batch in enumerate(tqdm(dataloader, desc='DataLoader')):
        ...
        pred = model(src_seq, class_label)
    ...
    if epoch == 1:
        torch.save(model, '../models/initial_classification_model_2_0.pt')
        torch.save({
        ...
        }, '../models/initial_classification_model_2_0_state_dict.pth')


    if len(acc_) > 0 and accuracy_mean > max(acc_):

        torch.save(model, '../models/classification_model_2_0_best.pt')
        torch.save({
            "model_dict": model.state_dict(),
            "optimizer_dict": optimizer.state_dict(),
            "epoch": epoch,
            ...
            "loss_class": loss_class, 
            "loss_subclass": loss_subclass
        }, '../models/classification_model_2_0_state_dict_best.pth')
